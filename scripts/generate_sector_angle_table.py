...
Binary parsing of #MRZ datagrams is handled by mbes_tools.kmall (iter_mrz_datagrams).
This script owns the application-layer logic: geometry masking, ping QC, flat-seafloor
filtering, angle binning, aggregation, and CSV export.
...
DATA_ROOT:
    Set DATA_ROOT in a .env file (copy .env.example) to establish the default
    input path.  --input overrides it explicitly.

Notes:
- KMALL depthMode values >= 100 indicate a manually selected depth mode.
  By default this script subtracts 100, so rawDepthMode 106 becomes depthMode 6.
...
def apply_flat_filter_to_ping(
    ...
    z = a*x + b*y + c is fit.  slope_deg = atan(sqrt(a²+b²)), roughness_m = RMS
    ...
    if radius_m <= 0:
        raise ValueError("--flat-radius-m must be positive")
    if min_neighbors < 3:
        raise ValueError("--flat-min-neighbors must be at least 3 for a plane fit")
